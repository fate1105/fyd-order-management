...
export default function QuickViewModal({ product, onClose, onAddToCart }) {
  ...
  const uniqueColors = [...new Map(variants.map(v => [v.colorId, { id: v.colorId, name: v.color, hex: v.colorHex }])).values()];
  const uniqueSizes = [...new Map(variants.map(v => [v.sizeId, { id: v.sizeId, name: v.size }])).values()];
